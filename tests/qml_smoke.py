...
for path in [root / "Service.qml", root / "Host.qml", root / "shell.qml", *sorted((root / "Commons").glob("*.qml")), *sorted((root / "Ui").glob("*.qml")), *sorted((root / "qml").glob("*.qml")), *sorted((root / "examples").rglob("*.qml")), *sorted((root / "sdk/starter").glob("*.qml"))]:
