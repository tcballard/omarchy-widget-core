...
for syntax in ['lua','conf']:
 for mode in ['success','legacy','conflict','reload-failure','inactive']:
  with tempfile.TemporaryDirectory() as tmp:
   ...
   if mode=='legacy':
    old='o.bind("SUPER + CTRL + SHIFT + W", "Desktop widgets", "omarchy-widget manage")' if syntax=='lua' else 'bindd = SUPER CTRL SHIFT, W, Desktop widgets, exec, omarchy-widget manage'
    target.write_text(original+old+'\n')
...
if mode=='conflict':print(json.dumps([{'modmask':72,'key':'W','keycode':0,'dispatcher':'exec','arg':'another-app'}]))
elif mode!='inactive' and ('SUPER + ALT + W' in target.read_text() or 'SUPER ALT, W' in target.read_text()):print(json.dumps([{'modmask':72,'key':'W','keycode':0,'dispatcher':'exec','arg':'omarchy-widget manage'}]))
else:print('[]')
''');ctl.chmod(0o755)
   ...
   if mode in ['success','legacy']:
    ...
    saved=target.read_text();assert 'CTRL' not in saved
    assert saved.startswith(original) and saved.count('omarchy-widget manage')==1
