...
- Content-first idle surface. For API 3, declare a settings editor and let Core supply its accessible top-right gear. API 1/2 compatibility content may call widgetContext.requestConfigure(). Keep configuration in the separate settings window. Core owns Save/Cancel, errors and acknowledgement. API 3 editors receive a shared Core-owned 32-unit corner gear. It reserves no header band. Leave its top-right corner clear; do not add a second gear.
