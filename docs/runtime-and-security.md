...
The September 17 live baseline (before these changes) measured one World Clock plus a closed resident manager at 237.4 MiB mean combined PSS and 2.39% of one core. The manager Quickshell process accounted for 134.4 MiB PSS. Removing that resident process is expected to reduce idle memory substantially, but the savings and CPU change must be measured on the same desktop; these are not post-fix results.
