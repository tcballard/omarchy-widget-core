...
pub fn client(path: &str, args: &[String]) -> Result<Value> {
    ...
    stream.set_read_timeout(Some(Duration::from_secs(8))).map_err(err)?;
    stream.set_write_timeout(Some(Duration::from_secs(2))).map_err(err)?;
    ...
    stream.take(RESPONSE_LIMIT + 1).read_to_end(&mut bytes).map_err(err)?;
    if bytes.len() > RESPONSE_LIMIT as usize { return Err("Broker response too large".into()); }
...
fn scoped(r: &Registry, package: &str, source: &Path, args: &[String]) -> Result<Value> {
    ...
    let layout = r.layout()?;
    if r.source(&layout, package)? != source { return Err("Runner generation expired".into()); }
    match args.first().map(String::as_str) {
        Some("list") if args.len() == 1 => {
            let mut snapshot = r.snapshot()?;
            snapshot["installed"].as_array_mut().ok_or("Invalid snapshot")?.retain(|v| v["packageId"] == package);
            for entry in snapshot["installed"].as_array_mut().unwrap() { entry["directory"] = json!("/widget"); }
            snapshot["problems"] = json!([]);
            snapshot["runtime"]["managerOpen"] = json!(false);
            let edit = snapshot["runtime"]["edit"]["instance"].as_str().unwrap_or("");
        ...
        Some("control") if args.len() == 2 && ["arrange", "finish-arrange"].contains(&args[1].as_str()) => r.control(&args[1]),
...
fn serve(listener: &UnixListener, r: &Registry, package: &str, source: &Path) -> Result<()> {
    ...
    for _ in 0..1 {
        ...
        stream.set_read_timeout(Some(Duration::from_millis(100))).map_err(err)?;
        stream.set_write_timeout(Some(Duration::from_millis(100))).map_err(err)?;
        let result = (|| {
            let mut bytes = Vec::new();
            (&mut stream).take(REQUEST_LIMIT + 1).read_to_end(&mut bytes).map_err(err)?;
            if bytes.len() > REQUEST_LIMIT as usize { return Err("Broker request too large".into()); }
            let args: Vec<String> = serde_json::from_slice(&bytes).map_err(err)?;
            scoped(r, package, source, &args)
        })();
        let response = result.unwrap_or_else(|e| json!({"error":e.chars().take(240).collect::<String>()}));
...
pub fn wayland_policy(args: &[String], message: &str) -> Result<Value> {
    if message.len() > REQUEST_LIMIT as usize { return Err("Wayland policy payload too large".into()); }
    let v: Value = serde_json::from_str(message).map_err(err)?;
    let allowed = match args.get(1).map(String::as_str).zip(args.get(2).map(String::as_str)) {
        ...
        Some(("zwlr_layer_surface_v1", "set_keyboard_interactivity")) => v["keyboard_interactivity"] == 0 || v["keyboard_interactivity"] == 2,
        _ => false,
    };
    if allowed { Ok(json!(true)) } else { Err("Wayland capability denied".into()) }
}
...
impl Drop for Runner {
    fn drop(&mut self) {
        ...
        let _ = self.proxy.kill(); let _ = self.proxy.wait();
...
fn launch(config: &Path, base: &Path, package: &str, source: &Path, upstream: &Path) -> Result<Runner> {
    ...
    let policy = format!("[socket]\nlisten = {}\nupstream = {}\n[exec]\nask_cmd = {}\n{}", toml_string(&display)?, toml_string(upstream)?, toml_string(&config.join("wayland-policy"))?, fs::read_to_string(config.join("wayland-filter.toml")).map_err(err)?);
    let policy_path = directory.join("wayland.toml");
    fs::write(&policy_path, policy).map_err(err)?;
    let mut proxy = Command::new(config.join("bin/wl-mitm")).arg(&policy_path)
        .env("TOKIO_WORKER_THREADS", "1").stdin(Stdio::null()).spawn().map_err(err)?;
    let ready = (|| {
        for _ in 0..100 {
            if proxy.try_wait().map_err(err)?.is_some() { return Err("Wayland filter exited before ready".into()); }
            if std::os::unix::fs::FileTypeExt::is_socket(&fs::symlink_metadata(&display).map(|m| m.file_type()).unwrap_or_else(|_| fs::metadata(&directory).unwrap().file_type())) { return Ok(()); }
            std::thread::sleep(Duration::from_millis(20));
        }
        Err("Wayland filter readiness timed out".into())
    })();
    if let Err(e) = ready { let _ = proxy.kill(); let _ = proxy.wait(); return Err(e); }
    let child = Command::new("/usr/bin/bash").arg(config.join("sandbox-launch"))
        .arg("runner").arg(source).arg(&socket).arg(&display)
        .stdin(Stdio::null()).spawn();
    let child = match child { Ok(v) => v, Err(e) => { let _ = proxy.kill(); let _ = proxy.wait(); return Err(err(e)); } };
    Ok(Runner { source:source.into(), directory, listener, proxy, child })
}
...
pub fn supervise(config: &Path) -> Result<Value> {
    ...
    if !runtime.is_absolute() || !safe_relative(&display) || display.contains('/') { return Err("Invalid Wayland environment".into()); }
    ...
    let mut manager = Command::new("/usr/bin/qs").args(["--no-duplicate", "-p"])
        .arg(config).env("OMARCHY_WIDGET_ROLE", "manager").env_remove("OMARCHY_WIDGET_BROKER")
    ...
    let result = (|| {
        ...
        loop {
            if manager.try_wait().map_err(err)?.is_some() { return Err("Widget manager exited".into()); }
            if Instant::now() >= next_scan {
                ...
                for entry in snapshot["installed"].as_array().ok_or("Invalid snapshot")? {
                    if entry["placement"]["enabled"] == true || entry["instanceId"] == edit {
                        wanted.insert(entry["packageId"].as_str().ok_or("Missing package")?.to_owned(), PathBuf::from(entry["directory"].as_str().ok_or("Missing source")?));
                    }
                }
                runners.retain(|id, runner| wanted.get(id) == Some(&runner.source));
                for (id, source) in wanted {
                    if runners.contains_key(&id) { continue; }
                    if let Some((count, when)) = failures.get(&id) {
                        if *count >= 3 || when.elapsed() < Duration::from_secs(5) { continue; }
                    }
                    match launch(config, &base, &id, &source, &upstream) {
                        Ok(runner) => { runners.insert(id, runner); }
                        Err(e) => {
            ...
            for (id, runner) in &mut runners {
                if runner.child.try_wait().map_err(err)?.is_some() || runner.proxy.try_wait().map_err(err)?.is_some() {
                    dead.push(id.clone()); continue;
                }
    ...
    drop(runners);
    let _ = manager.kill(); let _ = manager.wait();
...
mod tests {
    ...
    fn layer_policy_denies_privileged_requests() {
        for (interface, request, field, allowed, denied) in [
            ("zwlr_layer_shell_v1", "get_layer_surface", "layer", vec![1], vec![0,2,3]),
            ("zwlr_layer_surface_v1", "set_layer", "layer", vec![1], vec![0,2,3]),
            ("zwlr_layer_surface_v1", "set_exclusive_zone", "zone", vec![-1,0], vec![1,1920]),
            ("zwlr_layer_surface_v1", "set_keyboard_interactivity", "keyboard_interactivity", vec![0,2], vec![1,3]),
        ] {
            let args = ["wayland-policy", interface, request].map(str::to_owned);
            for value in allowed { assert!(wayland_policy(&args, &json!({field:value}).to_string()).is_ok()); }
            for value in denied { assert!(wayland_policy(&args, &json!({field:value}).to_string()).is_err()); }
    ...
    fn broker_scopes_reads_writes_and_generations() {
        ...
        let r = Registry { data:base.join("data"), state:base.join("state") };
        for id in ["io.test.a", "io.test.b"] {
            let source = base.join(id); fs::create_dir(&source).unwrap();
            ...
            r.install(&source).unwrap(); r.placement(id,"add",None).unwrap();
        }
        let source = r.source(&r.layout().unwrap(), "io.test.a").unwrap();
        let call = |args: &[&str]| scoped(&r,"io.test.a",&source,&args.iter().map(|s|s.to_string()).collect::<Vec<_>>());
        ...
        for args in [vec!["save","io.test.b",r#"{"revision":0,"settings":{}}"#],vec!["hide","io.test.b"],vec!["remove","io.test.a"],vec!["install","/widget"],vec!["configure","io.test.a","{}"]] { assert!(call(&args).is_err()); }
